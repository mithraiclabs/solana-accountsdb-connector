...
-- The table storing PsyOptions Tokenized Euros EuroMeta accounts

CREATE TABLE euro_meta_write (
    pubkey_id BIGINT NOT NULL REFERENCES pubkey,
    slot BIGINT NOT NULL,
    write_version BIGINT NOT NULL,
    underlying_mint BIGINT REFERENCES pubkey,
    underlying_decimals UINT1,
    underlying_amount_per_contract UINT8,
    stable_mint BIGINT REFERENCES pubkey,
    stable_decimals UINT1,
    stable_pool BIGINT REFERENCES pubkey,
    pyth_oracle BIGINT REFERENCES pubkey,
    strike_price UINT8,
    price_decimals UINT1,
    call_option_mint BIGINT REFERENCES pubkey,
    call_writer_mint BIGINT REFERENCES pubkey,
    put_option_mint BIGINT REFERENCES pubkey,
    put_writer_mint BIGINT REFERENCES pubkey,
    underlying_pool BIGINT REFERENCES pubkey,
    expiration INT8,
    bump_seed UINT1,
    expiration_data BIGINT REFERENCES pubkey,
    padding BYTEA,
    PRIMARY KEY (pubkey_id, slot, write_version)
);
CREATE INDEX euro_meta_write_searchkey on euro_meta_write(pubkey_id, slot DESC, write_version DESC);
CREATE INDEX euro_meta_write_pubkey_id_idx on euro_meta_write(pubkey_id);

-- The table storing PsyOptions Tokenized Euros ExpirationData accounts

CREATE TABLE expiration_data_write (
    pubkey_id BIGINT NOT NULL REFERENCES pubkey,
    slot BIGINT NOT NULL,
    write_version BIGINT NOT NULL,
    expiration INT8,
    pyth_oracle BIGINT REFERENCES pubkey,
    price_at_expiration UINT8,
    price_set_at_time INT8,
    price_decimals UINT1,
    price_set BOOL,
    bump  UINT1,
    padding BYTEA,
...
CREATE INDEX expiration_data_write_searchkey on expiration_data_write(pubkey_id, slot DESC, write_version DESC);
CREATE INDEX expiration_data_write_pubkey_id_idx on expiration_data_write(pubkey_id);
